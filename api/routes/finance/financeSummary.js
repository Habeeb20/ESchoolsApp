...
financeSummaryRoute.get('/summary', isAuthenticated, async(req, res) => {
    try {
        ...
        const expenditures = await ExpenditureFinance.find({ userId: req.userId });

        const totalAmountPaid = students.reduce((sum, student) => sum + student.amountPaid, 0);
        const totalExpenditure = expenditures.reduce((sum, exp) => sum + exp.amount, 0);
        const balance = totalAmountPaid - totalExpenditure;
        const numberOfStudentPaid = students.filter(student => student.amountPaid > 0).length;
        const totalOutstanding = students.reduce((sum, student) => sum + (student.amountPaid === 0 ? student.amountPaid : 0), 0);
